...
class Dataset():
    ...
    def generator(self, b_s, phase, show_ids=False, with_shots=False):
        ...
        snippet_ids, captions = self.get_annotations(phase)[:1000]
        captions = self.tokenizer.texts_to_sequences(captions)[:1000]
        ...
        with open(self.csv_file, 'r') as f:
            ...
            for i, row in enumerate(reader):
                ...
                if i > 0 and row[6] == 'English':  # and row[4] == 'clean':
                    ...
                    captions.append(
                        'STARTTOKEN ' + row[-1].encode('ascii', 'ignore')[:self.max_caption_len - 2] + ' STOPTOKEN')
