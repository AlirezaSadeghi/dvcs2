...
def model(video, caption):
    embedding_layer = Embedding(
        dataset.vocabulary_size, dataset.embedding_size, input_length=dataset.max_caption_len, mask_zero=True
    )
    video_embedding = TimeDistributed(Dense(dataset.embedding_size))(Masking()(Dropout(0.8)(video)))
    ...
    caption_encoding = Dropout(0.8)(embedding_layer(caption))
    decoder = GRU_Decoder(1024, dataset.embedding_size)([video_encoding_and_shots2, caption_encoding])
    out = TimeDistributed(Dense(dataset.vocabulary_size, activation='softmax'))(Dropout(0.8)(decoder))
    return out
...
def loss(y_true, y_pred):
    ...
    return K.sum(
        selettore * K.sparse_categorical_crossentropy(
            y_pred[:, :dataset.max_caption_len - 1],
            y_true[:, 1:, 0]
        )
    ) / K.sum(selettore)
...
def sparse_categorical_accuracy(y_true, y_pred):
    ...
    return K.sum(
        selettore * K.equal(
            y_true[:, 1:, 0],
            K.cast(K.argmax(y_pred[:, :dataset.max_caption_len - 1], axis=-1),
                   K.floatx())
        )
    ) / K.sum(selettore)
...
exp_name = 'experiments/my_model_1_lr_0.1_dr_0.8'


def trainig():
    history = m.fit_generator(dataset.generator(b_s, 'train'), dataset.nb_train_samples, 300,
                              validation_data=dataset.generator(b_s, 'val'), nb_val_samples=dataset.nb_val_samples,
                              callbacks=[
                                  ModelCheckpoint(exp_name + '_.{epoch:02d}-{val_loss:.2f}.pkl',
                                                  save_best_only=True)])
...
def test():
    ...
    while seen < dataset.nb_test_samples:
        ...
        for pred, gt, snippet_id in zip(X_caption, Y_gt, snippet_ids):
            ...
            if not any(h for h in hypothesis_for_coco if h['image_id'] == snippet_id):
                ...
                hypothesis_for_coco.append({"image_id": snippet_id, "id": seen, "caption": ' '.join(
                    [c.decode('utf-8').encode('ascii', 'ignore') for c in pred_caption])})
            references_for_coco.append({"image_id": snippet_id, "id": seen, "caption": ' '.join(
                [c.decode('utf-8').encode('ascii', 'ignore') for c in gt_caption])})
    ...
    json.dump(
        {'images': images_for_coco, 'annotations': references_for_coco, 'type': 'captions', 'info': {}, 'licenses': []},
...
if __name__ == "__main__":
    trainig()
    # call test() for test mode. Can later be improved to use ArgParser
