...
class StochasticStep(gof.Op):
    ...
    def perform(self, node, input_storage, output_storage):
        x, = input_storage
        sm = (1 / (1 + np.exp(-5*x))).astype(x.dtype)
...
class Step(gof.Op):
    ...
    def perform(self, node, input_storage, output_storage):
        x, = input_storage
        sm = (1 / (1 + np.exp(-x))).astype(x.dtype)
        output_storage[0][0] = sm
...
class BA_LSTM(Recurrent):
    ...
    def step(self, input, states):
        ...
        # Dropout values for U (Hidden state's learn-able matrix)-added automatically to the end of 2nd return value
        B_U = states[3]
        # Dropout values for X (Input's learn-able matrix)-added automatically to the end of 2nd return value (states)
        B_W = states[4]

        x_s = K.dot(x * B_W[4], self.W_s) + self.b_s
        s_l_begin = K.in_train_phase(stochastic_switch(x_s + K.dot(states[0] * B_U[4], self.U_s)) * s_lm1_end,
                                     switch(x_s + K.dot(states[0] * B_U[4], self.U_s)) * s_lm1_end)
    ...
    def call(self, x, mask=None):
        ...
        if self.return_sequences:
            ...
            shots = states[-1] * mask
            fp = K.zeros_like(shots[:, T.arange(1)])
            sp = mask[:, T.arange(self.T - 1)] - mask[:, T.arange(1, self.T)]
            shots += K.concatenate([fp, sp], axis=1)
